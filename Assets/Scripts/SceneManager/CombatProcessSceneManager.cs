...
namespace Kirara
{
    ...
    {
        private static bool GetIsCrit(CharacterModel ch)
        {
            return Random.Range(0f, 1f) <= ch.ae.GetAttr(EAttrType.CritRate).Evaluate();
        }

        private static float CalcDamage(CharacterModel ch, float dmgMult, bool isCrit)
        {
        ...
        private static float CalcDaze(CharacterModel ch, float dazeMult)
        {
            float daze = ch.ae.GetAttr(EAttrType.Impact).Evaluate() * dazeMult;
            return daze;
        }

        private static void CalcNumeric(CharacterModel ch, float dmgMult, float dazeMult,
